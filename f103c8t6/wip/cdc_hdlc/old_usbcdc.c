...
//	for (x=0;x<len;++x){xQueueSend(usb_rxq,&buf[x],0);}
//}
    ...
    uint8_t ep __attribute__((unused)))
{
    ...
    for (int i = 0; i < len; ++i) {
        if (xQueueSend(usb_rxq, &buf[i], 0) != pdPASS) {
            /* RX queue full: discard remaining bytes */
            break;
        }
    }
}
...
static void usb_task(void *arg)
{
    ...
    for (;;) {
        ...
        /*
         * Give other FreeRTOS tasks time to run.
         * USB still gets polled every millisecond.
         */
        vTaskDelay(pdMS_TO_TICKS(1));
    }
}
...
// 1: at least one char waiting for read
// 0: nothing to read
// -1 error
int usb_peek(void) {
char ch;
...
{
    while (bytes--) {
        if (xQueueSend(usb_txq, buf, 0) != pdPASS)
            return false;
...
bool usb_putc_nb(char ch)
{
    ...
    if (ch == '\n') {
        ...
        if (xQueueSend(usb_txq, &cr, 0) != pdPASS)
            return false;
    }

    return xQueueSend(usb_txq, &ch, 0) == pdPASS;
}


void usb_puts(const char *buf) {
while ( *buf ) usb_putc(*buf++);
}

//blocking
void usb_putc(char ch) {
static const char cr = '\r';
while ( !usb_ready() ) taskYIELD();
if ( ch == '\n' )
